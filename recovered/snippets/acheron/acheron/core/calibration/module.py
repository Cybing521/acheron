...
# File: tmpklho_618.marshal (Python 3.11)
...
def get_channel_setting_values(settings_len, cal = None, unit_type = None, scale = None, offset = ('settings_len', int, 'cal', asphodel.ChannelCalibration, 'unit_type', int, 'scale', float, 'offset', float, 'return', tuple[(dict[(int, int)], dict[(int, float)])])):
    unit_settings = {
...
def update_nvm(nvm, settings = None, unit_settings = None, float_settings = None, logger = ('nvm', bytes, 'settings', list[asphodel.AsphodelSettingInfo], 'unit_settings', dict[(int, int)], 'float_settings', dict[(int, float)], 'logger', DeviceLoggerAdapter, 'return', bytearray)):
    nvm = bytearray(nvm)
