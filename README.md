...
建议优先使用仓库内的 `conda` 环境。
这套脚本会用 `conda` 提供 `Python 3.11`，并优先复用本机已有的 `PySide6 6.10.x`；如果没有，再走 `pip` 安装。脚本还会额外生成一个可见的 `qt-runtime/plugins` 镜像，避开 macOS 上 Qt 对隐藏目录插件扫描不稳定的问题，也能减少重复下载超大的 Qt wheel。
